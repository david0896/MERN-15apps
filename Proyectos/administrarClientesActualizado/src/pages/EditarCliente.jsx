...
import { obtenerClienteXid, actualizarCliente } from "../api/clientes"
import Formulario from "../components/Formulario";
import Error from "../components/Error";


export async function loader({params}){
   const cliente = await obtenerClienteXid(params.clienteId);
   if(Object.values(cliente).length === 0){
        throw new Response('', {
            status: 404,
            statusText: 'Cliente no encontrado Upsss x('
        })
   }
   return cliente;
}
      ...
      <p className=" mt-3">Edita los datos del cliente</p>
        <div className='flex justify-end'>
            ...
            bg-blue-800 uppercase font-bold text-white py-1 px-3 
                ...
                onClick={()=>navigate(-1)}> {/*tambien se puede usar en el navigate el -1 para volver a la pagina anterior Ej: navigate (-1) */}
            volver 
            </button>
        </div>
        <div className=" bg-white shadow-md rounded-md md:w-3/4 mx-auto px-5 py-10 mt-10">
            ...
            <Form 
            method="post"
            noValidate //desactiva la validacion nativa de html5 para que puedarra correr la nuestra
            >
